...
def setup(exp,run,var,stim,user):
    ...
    user.postbuff = 150
    user.fs = 44100
    user.kwp = 0 # A placeholder. Will be set in pre_trial with the #keywords possible, then will be written out to datafile.
...
def pre_block(exp,run,var,stim,user):
    run.block_kwPossible = 0.
    run.block_kwCorrect = 0.
...
def pre_trial(exp,run,var,stim,user):
    stimname = var.current['target']
    p = "Trial "+ str(run.trials_exp+1) + ", " + stim.current[stimname]['filebase']+" KW: "+str(stim.current[stimname]['kw']) +"\n"+stim.current[stimname]['txt']
    stim.out, fs = m.read_file(stim.current[stimname]['file'])
    run.kwPossible = str(stim.current[stimname]['kw'])
    #stim.out = stim.current[stimname]['data']
    exp.interface.updateInfo_Trial(p)
    user.kwp = stim.current[stimname]['kw']
...
def post_trial(exp,run,var,stim,user):
    if run.gustav_is_go:
        trialPercent = round(float(run.response) / float(run.kwPossible) * 100, 1)
        run.block_kwPossible += float(run.kwPossible)
        run.block_kwCorrect += float(run.response)
        t = 'Trial: %s / %s (%s %%)\n' % (run.response, run.kwPossible, trialPercent)
    else:
        t = 'Trial unfinished (Exp cancelled)\n'    
    blockPercent = round(run.block_kwCorrect / run.block_kwPossible * 100, 1)
    exp.interface.updateInfo_TrialScore(t + 'Total: %0.0f / %0.0f (%0.1f %%)' % 
    (run.block_kwCorrect, run.block_kwPossible, blockPercent))

def post_block(exp,run,var,stim,user):
    blockPercent = round(run.block_kwCorrect / run.block_kwPossible * 100, 1)
    exp.interface.updateInfo_BlockScore("Prev Condition # %g\nScore: %s / %s (%s %%)" % 
    (run.condition+1, run.block_kwCorrect, run.block_kwPossible, blockPercent))
